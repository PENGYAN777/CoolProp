...
mesh0 = pd.read_csv("m0new.csv", ",", skiprows=0)
...
# axes.plot(mesh4.iloc[:,-6] , mesh4.iloc[:,7]/Pc, 'b', lw=lwh, label="30k")
...
# axes.plot(mesh4.iloc[:,-6] , mesh4.iloc[:,3], 'b', lw=lwh, label="30k")
# axes.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
...
# axes.plot(mesh4.iloc[:,-6] , mesh4.iloc[:,1]/dc, 'b', lw=lwh, label="30k")
...
axes.plot(mesh0.iloc[:,-6] , 1-mesh0.iloc[:,-3]-1/mesh0.iloc[:,3]/mesh0.iloc[:,3], 'k', lw=lwh, label="5k")

# axes.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
axes.set_xlabel('$X[mm]$',fontsize=12)
axes.set_ylabel('J',fontsize=12) 
# axes.set_title('entropy along y$=0.25 mm$',fontsize=14)
axes.legend(loc=1) # 
