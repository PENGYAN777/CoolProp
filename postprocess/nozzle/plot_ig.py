...
axes.plot(ig.iloc[:,5]*1000 , ig.iloc[:,2]/1e5, 'k', lw=2, label="PIG")
axes.plot(pr.iloc[:,5]*1000 , pr.iloc[:,2]/1e5, 'r', lw=2, label="PR")
axes.plot(cp.iloc[:,5]*1000 , cp.iloc[:,2]/1e5, 'b', lw=2, label="CoolProp")
axes2=axes.twinx()
...
# axes.set_ylim(0.2,1)
#axes.legend(loc=2) # 2 means left top
fig1.savefig("nozzle_rho_ig.pdf")
